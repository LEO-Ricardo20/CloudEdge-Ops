...
async function verify(authMode, browser) {
  ...
  try {
    ...
    await page.waitForFunction(() => document.querySelector('.device-item .status.online'));
    await page.locator('#device-status-filter').selectOption('unknown');
    assert.equal(await page.locator('.device-item').count(), 1);
    await page.locator('#device-status-filter').selectOption('all');
    ...
    assert.equal(await page.locator('#resolve-dialog').evaluate((node) => node.scrollWidth <= node.clientWidth), true);
    const screenshotDirectory = path.resolve(__dirname, '..', 'docs/screenshots');
    fs.mkdirSync(screenshotDirectory, { recursive: true });
    await page.screenshot({ path: path.join(screenshotDirectory, `${authMode}-disposition-mobile.png`) });
    await page.locator('#resolution-note').fill('   ');
    await page.locator('#resolve-submit').click();
    await page.locator('#resolve-error').waitFor({ state: 'visible' });
    await page.locator('#resolution-note').fill('已检查模拟采样，记录处置结果，持续观察温度趋势。');
    await page.locator('#resolve-submit').click();
    await page.waitForFunction(() => document.querySelector('#alerts').textContent.includes('已关闭'));
    await page.setViewportSize({ width: 1440, height: 1000 });
    assert.match(platform.listAlerts()[0].resolutionReason, /已检查模拟采样/);
    await page.locator('#telemetry-metric').selectOption('vibrationMmS');
    await page.waitForFunction(() => document.querySelector('#telemetry-heading').textContent === '振动速度趋势');
    await page.locator('#telemetry-metric').selectOption('temperatureC');
